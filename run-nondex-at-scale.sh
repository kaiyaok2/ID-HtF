...
function download_compile() {
    ...
    if [ $? == 0 ]; then # if build success
		...
		projects=$(./gradlew projects | grep Project | gcut -f3 -d" " | tr -d "':")
		...
		if [[ $sub == 0 ]]; then	# no subprojects
			...
			if ( grep "NonDex SUMMARY:" nondex.log ); then # if nondexTest is actually executed
				...
				if [[ $flaky_tests != '0' ]]; then 
					...
					sed -n -e '/Across all seeds:/,/Test results can be found at: / p' nondex.log | sed -e '1d;$d' | gcut -f1 -d' ' --complement | while read line
			...
			for p in ${projects}; do 
				...
				if ( grep "NonDex SUMMARY:" nondex:$p.log ); then # if nondexTest is actually executed
					...
					if [[ $flaky_tests != '0' ]]; then
						sha=$(git rev-parse HEAD)
						sed -n -e '/Across all seeds:/,/Test results can be found at: / p' nondex:$p.log | sed -e '1d;$d' | gcut -f1 -d' ' --complement | while read line
